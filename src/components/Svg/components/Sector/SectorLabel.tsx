...
function SectorLabel(props: ISvgSectorLabelProps): JSX.Element {
  ...
  const [directionVector, setDirectionVector] = useState<IPoint<number>>({
    x: 0,
    y: 0,
  })
  ...
  return (
    <Fragment>
      <SvgLine points={linePositions} strokeOptions={{ stroke: color }} />
      <SvgText {...props} point={position} textAnchor={textAnchor} />
    </Fragment>
  )
}

...
import { type IRectangle, type ISvgMouseCoordinates, SvgContext } from '../..'
import {
  defaultViewBox,
  type ISvgProps,
  SvgCurve,
...
const Svg = forwardRef(
  (
    ...
    const [mouseCoordinates, setMouseCoordinates] = useState<
      ISvgMouseCoordinates | undefined
    >()
    ...
    useEffect(() => {
      const minViewBoxSize = Math.min(
        viewBox.width - viewBox.x,
        viewBox.height - viewBox.y,
      )
    ...
    return (
      <SvgContext.Provider value={contextValue}>
        {/** biome-ignore lint/a11y/noSvgWithoutTitle: children will provide a title */}
        <svg
          ref={ref}
          className={twMerge(
            flipY && '-scale-y-100',
...
const forwardRefSvg: React.ForwardRefExoticComponent<
  ISvgProps & React.RefAttributes<SVGSVGElement>
> & {
